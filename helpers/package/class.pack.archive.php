...
require_once (WIS_SCRAPPER_PLUGIN_PATH.'helpers/package/class.pack.archive.filters.php');
require_once (WIS_SCRAPPER_PLUGIN_PATH.'helpers/package/class.pack.archive.zip.php');
require_once (WIS_SCRAPPER_PLUGIN_PATH.'lib/forceutf8/Encoding.php');
/**
...
class WIS_SCR_Archive
{
    ...
    public function __construct($package)
    {
    	...
    	$this->FilterInfo	= new WIS_SCR_Archive_Filter_Info();
    	$rootPath = WIS_SCR_Util::safePath(rtrim(WIS_SCRAPPER_WPROOTPATH, '//'));
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath("{$rootPath}/wp-admin");
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath(WP_CONTENT_DIR . "/uploads");
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath(WP_CONTENT_DIR . "/languages");
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath(WP_PLUGIN_DIR);
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath(get_theme_root());
    	$this->wpCorePaths[] = WIS_SCR_Util::safePath("{$rootPath}/wp-includes");
    }
    /**
    ...
    public function build($package)
    {
    	try {
    		...
    		$this->Package->setStatus(WIS_SCR_PackageStatus::ARCSTART);
    		switch ($this->Format) {
    			case 'TAR': break;
    			case 'TAR-GZIP': break;
    			default:
    			if (class_exists(ZipArchive)) {
    				$this->Format = 'ZIP';
    				WIS_SCR_Zip::create($this);
    			}
    			break;
    		}
    		$storePath  = "{$this->Package->StorePath}/{$this->File}";
    		$this->Size = @filesize($storePath);
    		$this->Package->setStatus(WIS_SCR_PackageStatus::ARCDONE);
    	} catch (Exception $e) {
     ...
     *
     *  @return obj Returns a SCR_Archive object
     */
	...
	public function saveActiveItem($package, $property, $value)
	{
		...
		return update_option(WIS_SCR_Package::OPT_ACTIVE, $package);
	}
	/**
	...
	public function parseDirectoryFilter($dirs = "")
	{
		...
		foreach ($dir_array as $val) {
			if (strlen($val) >= 2) {
				$clean_array[] = WIS_SCR_Util::safePath(trim(rtrim($val, "/\\"))) ;
			}
		}
     ...
     */
	 public function parseExtensionFilter($extensions = "")
	 {
	 	...
	 	if (strlen($extensions) >= 1 && $extensions != ";") {
	 		...
	 		$filter_exts = WIS_SCR_Util::appendOnce($extensions, ";");
	 	}
	 	return $filter_exts;
	 }
    /**
    ...
    private function createFilterInfo()
    {
    	...
    	if ($this->FilterOn) {
    		$this->FilterInfo->Dirs->Instance = array_map('WIS_SCR_Util::safePath', explode(";", $this->FilterDirs, -1));
    		$this->FilterInfo->Files->Instance = array_map('WIS_SCR_Util::safePath', explode(";", $this->FilterFiles, -1));
    		$this->FilterInfo->Exts->Instance = explode(";", $this->FilterExts, -1);
    	}
        /* FILTER: CORE ITMES */
        /* Filters Scrapper free packages & All pro local directories */
    	$wp_root	= rtrim(WIS_SCRAPPER_WPROOTPATH, '/');
    	...
    	$this->FilterInfo->Dirs->Core = array(
			/* WP-ROOT */
    		$wp_root . '/wp-wis-scrapper',
    		/* $wp_root . '/wp-scrapper-temp', */
			/* WP-CONTENT */
    		$wp_content . '/backups-scr-pro',
    	...
    	if (! WIS_SCR_Util::$PHP7_plus && WIS_SCR_Util::isWindows()) {
	...
	private function setDirFilters()
	{
		...
		foreach ($this->Dirs as $key => $val) {
			...
			if (! is_readable($this->Dirs[$key])) {
				$unset_key_list[] = $key;
				$this->FilterInfo->Dirs->Unreadable[] = WIS_SCR_Encoding::toUTF8($val);
			}
			...
			if ($invalid_test) {
				$utf8_key_list[] = $key;
				$this->FilterInfo->Dirs->Warning[] = WIS_SCR_Encoding::toUTF8($val);
			}
			/* Check for other WordPress installs */
			if ($name === 'wp-admin') {
				$parent_dir = realpath(dirname($this->Dirs[$key]));
				if ($parent_dir != realpath(WIS_SCRAPPER_WPROOTPATH)) {
		...
		foreach ($utf8_key_list as $key) {
			$this->Dirs[$key] =  WIS_SCR_Encoding::toUTF8($this->Dirs[$key]);
		}
	...
	private function setFileFilters()
	{
		...
		foreach ($this->Files as $key => $filePath) {
			...
			if ($invalid_test) {
				$utf8_key_list[] = $key;
				$filePath = WIS_SCR_Encoding::toUTF8($filePath);
				$fileName = basename($filePath);
				$this->FilterInfo->Files->Warning[] = array(
					'name'	=> $fileName,
			...
			$this->Size += $fileSize;
			if ($fileSize > WIS_SCRAPPER_SCAN_WARNFILESIZE) {
				$ext = pathinfo($filePath, PATHINFO_EXTENSION);
				$this->FilterInfo->Files->Size[] = array(
					'ubytes' => $fileSize,
					'bytes'  => WIS_SCR_Util::byteSize($fileSize, 0),
					'name'	 => $fileName,
					'dir'	 => pathinfo($filePath, PATHINFO_DIRNAME),
					'path'	 => $filePath);
			}
		...
		/* Try to repair utf8 paths */
		foreach ($utf8_key_list as $key) {
			$this->Files[$key] =  WIS_SCR_Encoding::toUTF8($this->Files[$key]);
		}
		/* Remove unreadable items outside of main loop for performance */
		if (count($unset_key_list)) {
			foreach ($unset_key_list as $key) {
	...
	private function setTreeFilters()
	{
		...
		$dir_group = WIS_SCR_Util::array_group_by($this->FilterInfo->Files->Size, "dir" );
		ksort($dir_group);
		foreach ($dir_group as $dir => $files) {
			...
			foreach ($this->wpCorePaths as $core_dir) {
				if (strpos(WIS_SCR_Util::safePath($dir), WIS_SCR_Util::safePath($core_dir)) !== false) {
					$iscore = 1;
					break;
				}
			}
			$this->FilterInfo->TreeSize[] = array(
				'size' => WIS_SCR_Util::byteSize($sum, 0),
				'dir' => $dir,
				'sdir' => str_replace(WIS_SCRAPPER_WPROOTPATH, '', $dir),
				'iscore' => $iscore,
				'files' => $files
			);
		}
		...
		$dir_group = WIS_SCR_Util::array_group_by($this->FilterInfo->Files->Warning, "dir" );
		ksort($dir_group);
		foreach ($dir_group as $dir => $files) {
			/* Locate core paths, wp-admin, wp-includes, etc. */
			$iscore = 0;
